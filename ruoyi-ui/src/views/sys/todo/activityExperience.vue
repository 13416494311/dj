<template>
  <div style="padding: 30px">
    ...
    <el-card shadow="always" style="margin-bottom: 30px;">
      <div slot="header" style="height: 25px">
        <span style="font-weight: bold;font-size: 16px">心得体会</span>
      </div>
      <el-form ref="activityExperience" :model="activityExperience" :rules="rules" label-width="150px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="标题" prop="title">
              <el-input v-model="activityExperience.title" placeholder="请输入标题"/>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <el-form-item label="心得体会" prop="content">
              <el-input v-model="activityExperience.content" type="textarea" :autosize="{ minRows: 4, maxRows: 4}"
                        placeholder="请输入内容"/>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    methods: {
      init() {
        ...
        getTodo(this.todoId).then(response => {
          ...
          this.reset();
          this.activityExperience.experienceUuid = this.uuid()
      ...
      reset() {
        ...
        this.resetForm("activityExperience");
        this.resetForm("form");
      },
      uploadFileNull(file) {
      ...
      uploadFile(file) {
        ...
        formData.append("uuid", this.activityExperience.experienceUuid);
        formData.append("file", file.file);
        formData.append("fileType", "activityExperience");
      ...
      getFileList() {
        this.fileList = [];
        listFile({'uuid': this.activityExperience.experienceUuid}).then(response => {
          let files = response.rows;
          for (let i = 0; i < files.length; i++) {
            let file = {};
            file.name = files[i].fileName;
      ...
      submitForm: function (status) {
        this.$refs["activityExperience"].validate(valid => {
          if (valid) {
            this.activityExperience.detailedUuid = this.form.detailedUuid;
            this.activityExperience.partyMemberId = this.partyMemberId;
            if (this.activityExperience.experienceId != undefined) {
              updateExperience(this.activityExperience).then(response => {
                if (response.code === 200) {
              ...
              addExperience(this.activityExperience).then(response => {
                if (response.code === 200) {
                  this.msgSuccess("提交成功");
                  this.submitTodo(status)
                } else {
