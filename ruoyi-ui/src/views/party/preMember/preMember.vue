  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="18" :xs="24">
        <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="150px">
          <el-form-item label="入党积极分子姓名" prop="memberName">
            <el-input
              v-model="queryParams.memberName"
              placeholder="请输入入党积极分子姓名"
              clearable
              size="small"
    ...
    <el-dialog :title="title" :visible.sync="open" width="90%" append-to-body
      ...
      <div slot="footer" class="dialog-footer" :style="{textAlign:'center'}">
        <el-button v-if="!disabled && form.memberId !=undefined " type="primary" @click="chooseAuditUser">转为预备党员</el-button>
        <el-button v-show="!disabled" type="primary" @click="submitForm">保 存</el-button>
        <el-button @click="cancel">取 消</el-button>
      </div>
    </el-dialog>

    <choose-audit-user ref="chooseAuditUser"  @ok="submitFormAudit" @del="handleDelete" @enable="handleEnable"/>
...
<script>
  ...
  export default {
    name: "PartyMember",
    components: {MemberPortrait, MemberEducation, Exemplary, Specialty, ChooseAuditUser, Treeselect,selectTree},
    data() {
    ...
    methods: {
      ...
      submitForm: function () {
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.memberId != undefined) {
              updatePartyMember1(this.form).then(response => {
                if (response.code === 200) {
              ...
              addPartyMember1(this.form).then(response => {
                if (response.code === 200) {
